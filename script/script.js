...
const clearTable = () => { //limpa as linhas para não ficar duplicando os elementos cada vez que atualizar a tabela
    ...
    if (rows.length == '') {
        var newRow = document.createElement('tr')
        document.querySelector('#tableClient>tbody').appendChild(newRow)
        newRow.innerHTML = 'Não há clientes cadastrados'
    }
}
...
const fillFields = (cliente) => { //quando clicar para editar tem que aparecer os dados do cliente para poder editar
    ...
    document.getElementById('datanasc').value = cliente.data
    ...
    if (event.target.type == 'button') {
        ...
        if (action == 'edit') {
            ...
            if (response) {
                deleteClient(index)
                updateTable()
                var rows = document.querySelectorAll('#tableClient>tbody tr')
                var newRow = document.createElement('tr')
                if (rows.length == '') {
                    document.querySelector('#tableClient>tbody').appendChild(newRow)
                    newRow.innerHTML = 'Não há clientes cadastrados'
